...
def draw_pose(pose, H, W, no_face=False):
    bodies = pose["bodies"]
    faces = pose["faces"]
    hands = pose["hands"]
    candidate = bodies["candidate"]
    ...
    # print('candidate',candidate, subset)
    # input('x')
    canvas = util.draw_bodypose(canvas, candidate, subset)
...
class DWposeDetector:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        no_face = kwargs.get('no_face') or False
        only_eye = kwargs.get('only_eye') or False
        ...
        with torch.no_grad():
            ...
            detected_map = draw_pose(pose, H, W,no_face)
            detected_map = HWC3(detected_map)
